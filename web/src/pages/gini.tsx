...
const Charts: React.FC<ChartsProps> = ({ defaultColor }) => {
  ...
  const handleSetFilter = (event: any) => {
    ...
    const selectedCountries = value
      .replace(/,\s/g, ",")
      .split(",")
      .map((country: any) => country.toLowerCase());
    if (value === "") {
      setFilter(undefined);
    } else {
  ...
  useEffect(() => {
    if (rawData) {
      ...
      const nameFiltered = filter
        ? rangeFiltered.filter((entry: any) =>
            filter.includes(entry.country.toLowerCase())
          )
        : rangeFiltered;
      const dataObject = {
        y: "Gini index",
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      const moved = (event: any) => {
        ...
        const s = least(data.countries, (d: any) =>
          Math.abs(+d.indexed[i] - ym)
        );
  ...
  return (
    <Layout defaultColor={defaultColor}>
      <Flex>
        <Box>
          <Box mb={2}>
            <Link
              ...
              href="https://data.worldbank.org/indicator/SI.POV.GINI"
              isExternal
            >
              World Bank Estimate
        ...
        <Box ml={2}>
          <Box mb={2}>
            <Text color={linkColor[colorMode]}>Filters</Text>
          </Box>
          <Box
            backgroundColor={bgColor[colorMode]}
            border="1px solid"
          ...
          >
            <Flex h="100%" flexDir="column">
              ...
              <Box
                ...
                height="100%"
                overflowY="scroll"
                className={scrollbar[colorMode]}
                border="1px solid"
                borderColor={bordColor[colorMode]}
              >
                {data
                  ? data.countries.map((entry: any, index: any) => (
                      <Box key={index}>
                        <PseudoBox
                          as="button"
                          _hover={{
                            bg: `${highlightColor[colorMode]}`,
                            color: "white",
                          }}
